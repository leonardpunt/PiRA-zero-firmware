...
from ..const import MEASUREMENT_DEVICE_VOLTAGE
from ..messages import get_measurement_data
...
class Messages:
    StartingUp, ShuttingDown, Catches = range(3)
...
class Module(object):
    ...
    def process(self, modules):
        ...
        self._send_catches(catches)

    def shutdown(self, modules):
        self._send_shutting_down()
    ...
    def _send_starting_up(self):
        ...
        self._transmit_lora_msg(self._create_lora_message(Messages.StartingUp, voltage))
        self._slack_msg("Starting up. Voltage: %s" % (voltage,))
    ...
    def _send_catches(self, catches):
        self._transmit_lora_msg(self._create_catches_lora_message(catches))
        self._slack_msg(self._create_catches_slack_message(catches))

    def _create_catches_lora_message(self, catches):
        if not catches:
            return
        return self._create_lora_message(Messages.Catches, len(catches))

    def _create_catches_slack_message(self, catches):
        if not catches:
            return "No new TMSIs/IMSIs found"
    ...
    def _transmit_lora_msg(self, msg):
        # Initialize lora module, if needed
        if not self._lora:
            self._initialize_lora_module()
        ...
        print("Transmitting message ({} bytes) via LoRa...".format(len(msg)))
    ...
    def _create_lora_message(self, msg_type, data=None):
        raw_msg = io.BytesIO()
        if data:
            raw_msg.write(struct.pack('!HH', msg_type, data))
        else:
            raw_msg.write(struct.pack('!H', msg_type))
        return raw_msg.getvalue()
